...
import 'package:ipsewallet/pages/my/staking/actions/rewardDetailPage.dart';
...
import 'package:ipsewallet/store/staking/types/ownStashInfo.dart';
...
import 'package:ipsewallet/widgets/outlined_circle.dart';
...
class _StakingActions extends State<StakingActions>
  ...
  Future<void> _updateStakingTxs() async {
    ...
    List res = await webApi.staking.updateStaking(_txsPage);
    if (mounted) {
      setState(() {
        _loading = false;
      });
    ...
    res.addAll(store.staking.txs.map((i) {
      return Container(
        ...
        child: ListTile(
          ...
          :Icon(Icons.cancel_rounded,color:Colors.pink,size:35),
          title: Text(i.call),
  ...
  Widget _buildActionCard() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: LinearGradient(colors: [Color(0xFFEFF0F2), Color(0xFFE9EEF2)]),
        boxShadow: [
          BoxShadow(color: Color(0x0F000000), offset: Offset(8, 0), blurRadius: 20, spreadRadius: 0)
        ]
      ),
      child: !hasData
          ? Container(
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          Text(
                            ...
                            style: TextStyle(fontSize: 15.0, color: Config.color999),
                          ),
                          Text(Fmt.address(store.account.currentAddress),style: TextStyle(fontSize: 13.0, color: Config.color999))
                        ],
                      ),
                    ),
                    Expanded(
                      child: Column(
                        ...
                        children: <Widget>[
                          Text(
                            '${Fmt.priceFloorBigInt(info?.total??BigInt.zero, decimals, lengthMax: 3)}',
                            style: TextStyle(fontSize: 24, color: Config.themeColor,
                ...
                Container(
                  ...
                  child: acc02 != null
                      ? AddressIcon(acc02.address,
                          pubKey: acc02.pubKey, size: 30)
                      : AddressIcon(address02, size: 30),
                ),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
