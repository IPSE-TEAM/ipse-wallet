...
abstract class _StakingStore with Store {
  ...
  @observable
  ObservableList<StakingTxData> txs = ObservableList<StakingTxData>();

  @observable
  ObservableList<StakingTxRewardData> txsRewards = ObservableList<StakingTxRewardData>();
  ...
  @action
  Future<void> addTxs(List res, {bool shouldCache = false}) async {
    if (res == null ) return;
    // txsCount = res['count'];

    List<StakingTxData> ls =
        List.of(res).map((i) => StakingTxData.fromJson(i)).toList();
  ...
  @action
  Future<void> addTxsRewards(List res, {bool shouldCache = false}) async {
    if (res == null) return;
    List<StakingTxRewardData> ls =
        List.of(res).map((i) => StakingTxRewardData.fromJson(i)).toList();
